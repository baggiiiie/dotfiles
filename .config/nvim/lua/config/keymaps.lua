...
map("n", "<leader>dv", "<cmd>NvDiff<cr>", { desc = "NvDiff" })
map("n", "<leader>df", "<cmd>NvDiff files<cr>", { desc = "NvDiff files" })
map("x", "<leader>da", function()
  require("config.nvim-diff-view").add_comment()
end, { desc = "Add review comment" })
map("n", "<leader>dr", "<cmd>NvDiff reviews<cr>", { desc = "NvDiff reviews" })
